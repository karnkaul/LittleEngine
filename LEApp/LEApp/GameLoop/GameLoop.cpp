...
#include "LEGame/GameModel/GameConfig.h"
...
bool Init(s32 argc, char** argv)
{
	...
	tickRate = Time::Seconds(1.0f / static_cast<f32>(config.GetTicksPerSecond()));
	maxFrameTime = Time::Milliseconds(config.GetMaxTickTimeMS());
...
#endif
#if ENABLED(PROFILER)
	Profiler::Init(*uContext, Time::Milliseconds(config.GetMaxTickTimeMS()));
#endif
	GameClock::Reset();
