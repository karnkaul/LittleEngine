...
namespace LittleEngine {
	...
	UIWidget* UIWidgetMatrix::Get() {
		if (m_matrix.IsEmpty() || m_matrix.GetRef().IsEmpty()) return nullptr;
		return m_matrix.GetRef().GetRef().get();
	}

	void UIWidgetMatrix::Up() {
		Vec& vec = GetCurrentVec();
		if (!vec.IsEmpty()) vec.Decrement();
	}

	void UIWidgetMatrix::Down() {
		Vec& vec = GetCurrentVec();
		if (!vec.IsEmpty()) vec.Increment();
	}
	...
	void UIWidgetMatrix::Reset(bool bResetRows) {
		if (m_matrix.IsEmpty() || GetCurrentVec().IsEmpty()) return;
		if (bResetRows) m_matrix.Reset();
		GetCurrentVec().Reset();
	}
