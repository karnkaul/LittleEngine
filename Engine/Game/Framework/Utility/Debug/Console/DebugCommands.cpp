...
#include "Game/Model/World/World.h"
...
void Add()
{
	...
	auto iter =
		std::find_if(commands.begin(), commands.end(), [&uT](const std::unique_ptr<Command>& uC) { return uT->m_name < uC->m_name; });
...
std::vector<LogLine> ExecuteQuery(const std::string& command, std::string params)
{
	...
	auto search = std::find_if(commands.begin(), commands.end(),
							   [&command](const std::unique_ptr<Command>& uCommand) { return uCommand->m_name == command; });
...
void AddCommand(std::unique_ptr<Command> uCommand)
{
	auto iter = std::find_if(commands.begin(), commands.end(),
							 [&uCommand](const std::unique_ptr<Command>& uC) { return uCommand->m_name < uC->m_name; });
