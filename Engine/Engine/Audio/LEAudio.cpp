...
LEAudio::LEAudio(std::string_view rootDir) : m_rootMusicDir(rootDir)
{
	...
	musicVol.BindCallback(
		[](std::string_view val) { AudioPlayer::s_mixVols[ToIdx(AudioPlayer::Mix::Music)] = Fixed(Strings::ToF32(std::string(val))); });
	sfxVol.BindCallback(
		[](std::string_view val) { AudioPlayer::s_mixVols[ToIdx(AudioPlayer::Mix::SFX)] = Fixed(Strings::ToF32(std::string(val))); });
#endif
}
