...
{
	std::shared_ptr<ManifestLoader> sLoader =
		std::make_shared<ManifestLoader>(*this, std::move(manifestPath), std::move(onComplete), false);
...
void LERepository::UnloadAll(bool bUnloadDefaultFont)
{
	...
	{
		Core::RemoveIf<std::string, std::unique_ptr<Asset>>(
			m_loaded, [](std::unique_ptr<Asset>& uAsset) { return std::string(uAsset->ID()) != g_pDefaultFont->ID(); });
	}
