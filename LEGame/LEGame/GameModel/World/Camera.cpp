...
{
Camera::Camera(LEContext& context) : m_pContext(&context)
{
}
...
void Camera::FillViewWithTiles(class TextureAsset& texture)
{
	if (!m_uTileMap)
	{
		m_uTileMap = MakeUnique<TileMap>(*m_pContext->Renderer()->New(LAYER_ZERO), true);
	}
	m_uTileMap->FillView(m_pContext->GetViewSize(), texture);
}
