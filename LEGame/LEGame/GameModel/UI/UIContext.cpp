...
void UIContext::OnCreate(GameManager& gameManager, String name, LayerID rootLayer)
{
...
{
	Assert(m_pGameManager, "GameManager is null!");
	m_inputTokens.clear();
	if (bActive)
	{
		if (bResetSelection)
		...
		Tick(Time::Zero);
		m_inputTokens.push_back(m_pGameManager->Input()->Register(
...
GameManager* UIContext::Game() const
{
