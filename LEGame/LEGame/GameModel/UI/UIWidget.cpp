...
#include "LEGame/GameModel/UI/UITransform.h"
#include "UIContext.h"
...
GameManager* UIWidget::Game() const
{
	return m_pOwner->Game();
}
...
void UIWidget::InitElement(String name, UIElement* pNewElement, UITransform* pParent)
{
	if (!pParent)
	{
		pParent = m_pOwner ? &m_pOwner->GetRoot()->m_transform : nullptr;
	}
	pNewElement->OnCreate(*m_pOwner->Game()->Context(), std::move(name), pParent);
