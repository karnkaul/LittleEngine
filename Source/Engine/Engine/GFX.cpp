...
const Vector2& GFX::UIViewSize() const
{
	return uiViewSize;
}
...
const Vector2& GFX::WorldViewSize() const
{
	return worldViewSize;
}
...
{
	return Projection(nPos, uiViewSize);
}

Vector2 GFX::WorldProjection(Vector2 nPos) const
{
	return Projection(nPos, worldViewSize);
}

Vector2 GFX::WorldToUI(Vector2 world) const
{
	Vector2 full = Projection(NormalisedPosition(world, worldViewSize), uiViewSize);
...
void GFX::Recompute()
{
...
#endif
	uiAspectRatio = uiViewSize.x.ToF64() / uiViewSize.y.ToF64();
	...
	Fixed worldWidth(worldViewHeight.ToF64() * nativeAspectRatio);
	worldViewSize = Vector2(worldWidth, worldViewHeight);
	uiViewCrop = sf::FloatRect(0.0f, 0.0f, 1.0f, 1.0f);
	if (uiAspectRatio < nativeAspectRatio)
	{
		f64 uiWidth = uiAspectRatio * worldViewHeight.ToF64();
		f64 widthRatio = (static_cast<f64>(viewportWidth) - uiWidth) / static_cast<f64>(viewportWidth);
	...
	else if (uiAspectRatio > nativeAspectRatio)
	{
		...
		f64 heightRatio = (viewportHeight.ToF64() - uiHeight) / viewportHeight.ToF64();
