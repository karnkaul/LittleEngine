...
{
	Vector2 space = mouseLayer >= LayerID::TopFull ? g_pGFX->WorldSpace() : g_pGFX->UISpace();
	m_pMouseH = m_pContext->Renderer()->New<Quad>(mouseLayer);
	m_pMouseV = m_pContext->Renderer()->New<Quad>(mouseLayer);
